...
// console.log(reports)
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        setLoading(true);
        const res = await endpointroute.get(`base/reports`);
        setReport(res.data);
        setNotFound(false);
      } catch (error) {
...
const summary = `base/summary?startDate=${fromDate}&endDate=${toDate}`;
...
setSummaryData(res?.data)
 setShowSummaryModal(true) 
    ...
    onsole.log('full report',res?.data)
  } catch (error) {
...
const handleSubmitToRegion = async () => {
  try {
    ...
    await endpointroute.post("base/summary/forward-edited", summaryData)
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 overflow-y-scroll">
    <div className="bg-white rounded-xl p-6 max-w-lg w-full space-y-4 shadow-lg">
      <h2 className="text-xl font-semibold text-center">Review & Edit Summary</h2>
<div className="md:grid grid-cols-2 gap-4">
      ...
      >
        {sending?"Submitting..":"Submit to Region"}
      </button>
...
<section className="bg-white border border-gray-300 rounded-lg p-5 mb-6">
  ...
  >
   ...
   <input
    ...
    reports.length > 2 ? "2xl:grid-cols-3" : "2xl:grid-cols-2"
  }`}>
