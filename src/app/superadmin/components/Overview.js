...
const Overview = () => {
    ...
    const fetchAllBookings = async () => {
      try {
        ...
        const [reportsRes, usersRes] = await Promise.all([
          endpointroute.get("superadmin/summaries"), 
          endpointroute.get("superadmin/region-admins"),
        ]);
        SetReports(reportsRes.data);
        setUsers(usersRes.data);
        console.log('report',reportsRes.data)
      } catch (error) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await endpointroute.put(`superadmin/region-admins/${selectedUser._id}/reset-password`, {
        newEmail,
      }); 
  ...
  const overviewStats = [
    {
      title: "Total Registered Regions",
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {loading ? (
            ...
            <p className="text-center col-span-full text-gray-500">No report found .</p>
          ) : (
            allZones.map((zone, index) => (
              <div key={index} className="bg-white border border-black rounded-xl p-5 shadow-sm">
                ...
                {/* <h2 className="text-lg font-bold text-blue-500">{zone.base}</h2> */}
                                <h2 className="text-lg font-bold text-blue-500">{zone?.region.toUpperCase()}</h2>

                <div className="grid gap-2">
                  <Link href={`/admin/reports/${zone._id}?name=${zone.region}`}>
      ...
      {!loading && filteredZones.length > 0 && (
        <div className="flex justify-center items-center gap-5 mb-6">
          ...
          <h2 className="text-sm font-medium text-black">
            Showing {total} of {filteredZones.length} Regions
          </h2>
          <button
            onClick={() => setCurrentPage(currentPage + 1)}
